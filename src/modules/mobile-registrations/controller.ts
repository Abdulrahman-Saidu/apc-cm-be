...
export const mobileRegistrationsController = {
  async create(req: Request, res: Response) {
    const result = await mobileRegistrationsService.create(
      req.user!.id,
      req.body
    );
  ...
  async sync(req: Request, res: Response) {
    const results = await mobileRegistrationsService.sync(
      req.user!.id,
  ...
  async listMine(req: Request, res: Response) {
    const status =
      (req.query.status as 'pending' | 'approved' | 'rejected' | 'all') ??
      'all';

    const data = await mobileRegistrationsService.listMine(
      req.user!.id,
      status
    );
